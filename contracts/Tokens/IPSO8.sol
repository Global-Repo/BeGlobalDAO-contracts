 ...
 */
contract IPSO8 is ReentrancyGuard, Ownable
{
    ...
    )
    {
        ...
        ratioRequiredWGLBDDen = wGlobalDAOToken(_wGLBD).sGLBDTowGLBD(_ratioRequiredWGLBDDen);
    ...
    {
        ratioRequiredWGLBDNum = _ratioRequiredWGLBDNum;
        ratioRequiredWGLBDDen = wGlobalDAOToken(wGLBD).sGLBDTowGLBD(_ratioRequiredWGLBDDen);
    }
    ...
    function canInvestMax(address _user) public view returns (uint)
    {
        ...
        if(hardCap > 0)
        {
            ...
            amountToInvest = amountToInvest < amountRemainingToInvest ? amountToInvest : amountRemainingToInvest;
        }
    ...
    {
        uint maxInvest = canInvestMax(_depositor);
        uint balance = IBEP20(investmentToken).balanceOf(_depositor);
        return maxInvest < balance ? maxInvest : balance;
    }

...
contract MockGlobalDAOTreasury is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sGLBDQueue; // Delays change to sGLBD address
...
@notice allow approved address to borrow reserves
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sGLBD).balanceOf(msg.sender); // Can only borrow against sGLBD held
...
@notice allow approved address to repay borrowed reserves with GLBD
        @param _amount uint
     */
    function repayDebtWithGLBD(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");
...
@notice returns GLBD valuation of asset
    ...
    returns (uint256 value_)
    {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IERC20(GLBD).decimals()).div(
...
@notice queue address to change boolean in mapping
    ...
    returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SGLBD) {
            // 9

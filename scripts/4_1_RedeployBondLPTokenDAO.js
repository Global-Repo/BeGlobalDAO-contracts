...
async function main() {
    ...
    // Initial Bond debt
    const intialGLBDBUSDBondDebt = '5802725553630'; //'1131577472670';
    ...
    glbdbusdLP = await GLBDBUSDLP.attach(GLBD_BUSD_LP_2_ADDRESS);
    await new Promise(r => setTimeout(() => r(), timeoutPeriod));
    ...
    glbdbusdBond = await GLBDBUSDBond.deploy(GLBD_ADDRESS, GLBD_BUSD_LP_2_ADDRESS, TREASURY_ADDRESS, MULTISIG_ADDRESS, BONDING_CALCULATOR_ADDRESS);
    ...
    await treasury.queue('5', GLBD_BUSD_LP_2_ADDRESS);
    await new Promise(r => setTimeout(() => r(), timeoutPeriod));

    // Toggle GLBD_BUSD_LP as liquidity token
    console.log("[Toggle GLBD_BUSD_LP as liquidity token]");
    await treasury.toggle('5', GLBD_BUSD_LP_2_ADDRESS, BONDING_CALCULATOR_ADDRESS);
    await new Promise(r => setTimeout(() => r(), timeoutPeriod));

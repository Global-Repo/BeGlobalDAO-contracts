...
const {
    ...
    STAKING_HELPER_ADDRESS, DEPLOYER_ADDRESS, STAKING_HELPER_ADDRESS_NOU, STAKING_ADDRESS_NOU
...
async function main() {
    ...
    await busdBond.setAdjustment(false,'0','10','0');
    console.log("[Success]");
    await new Promise(r => setTimeout(() => r(), timeoutPeriod));

    try {
        console.log("VERIFYING BUSD Bond only staking: ", busdBond.address);
        //// Verify contract on bsc
        await hre.run("verify:verify", {
            address: busdBond.address,
            constructorArguments: [GLBD_ADDRESS, BUSD_ADDRESS, TREASURY_ADDRESS, MULTISIG_ADDRESS, '0x0000000000000000000000000000000000000000'],
        });
        console.log( "BUSD Bond only staking verified: " + STAKING_HELPER_ADDRESS_NOU );
        console.log("Success");
    } catch (err) {
        console.log(err.message);
    }

    console.log("[BUSD Bond only staking deployed successfully]");
}
